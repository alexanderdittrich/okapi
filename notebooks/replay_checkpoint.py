...
def replay_checkpoint(
    ...
    if env_id is None:
        ...
        if len(path_parts) >= 2:
            ...
            raise ValueError("Could not infer environment ID from checkpoint path. Please specify env_id.")
    ...
    # Load checkpoint
    try:
        global_step, num_updates, loaded_key = load_checkpoint(checkpoint_path, model, optimizer)
    ...
    print(f"  Min/Max Return: {np.min(episode_returns):.1f} / {np.max(episode_returns):.1f}")
    print(f"  Average Length: {np.mean(episode_lengths):.1f} ± {np.std(episode_lengths):.1f}")
    
    # Environment-specific success metrics
    if "CartPole" in env_id:
        success_threshold = 195
        success_rate = sum(1 for r in episode_returns if r >= success_threshold) / len(episode_returns) * 100
...
def main():
    ...
    parser.add_argument(
        "--env-id",
        help="Environment ID (auto-detected if not specified)"
    )
