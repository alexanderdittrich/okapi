...
class PPOConfig:
    ...
    hidden_sizes: list[int] = None  # [64, 64] by default
        ...
        if self.is_discrete:
            ...
            actor_layers.append(
                nnx.Linear(
                    actor_in, 
                    action_dim, 
                    kernel_init=nnx.initializers.orthogonal(scale=0.01),  # Small init for policy
                    bias_init=nnx.initializers.constant(0.0),
                    rngs=rngs
                )
            )
...
def ppo_loss(
    ...
    v_loss = jax.lax.cond(
        clip_vloss,
        compute_clipped_v_loss,
        compute_unclipped_v_loss
    )
...
def train_step(
    ...
    # Clip gradients - use jax.lax.cond for JIT compatibility
    grad_norm = jnp.sqrt(sum(jnp.sum(g**2) for g in jax.tree.leaves(grads)))
    info["diagnostics/grad_norm"] = grad_norm

    def clip_grads():
        return jax.tree.map(lambda g: g * max_grad_norm / grad_norm, grads)
...
def train(cfg: PPOConfig):
    ...
    if cfg.norm_obs:
        ...
        envs = gym.wrappers.vector.TransformObservation(envs, lambda obs: np.clip(obs, -cfg.clip_obs, cfg.clip_obs))
    if cfg.norm_reward:
